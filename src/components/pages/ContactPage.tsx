...
export default function ContactPage(props: ContactPageData) {
  return (
    <PageShell activeNavLabel={props.activeNavLabel ?? "About"}>
      <section className="relative overflow-hidden px-6 pb-20 pt-20 lg:px-8 lg:pt-24">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {props.hero.previewCards.map((item) => (
              <div key={item.title} className="rounded-[1.75rem] border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                <div className="mb-3 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/75">Contact preview</div>
      ...
      <section className="bg-white px-6 py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionIntro {...props.pathsIntro} /><div className="mt-14 grid gap-5 md:grid-cols-3">{props.paths.map((item) => <FeatureCard key={item.title} {...item} />)}</div></div></section>
      <section className="bg-slate-50 px-6 py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionIntro {...props.whyIntro} /><div className="mt-14 grid gap-5 md:grid-cols-3">{props.whyCards.map((item) => <FeatureCard key={item.title} {...item} />)}</div></div></section>
