...
import ResourceFeaturedCard from "@/components/cards/ResourceFeaturedCard";
import ResourceCard from "@/components/cards/ResourceCard";
import FeatureCard from "@/components/cards/FeatureCard";
import { ResourceIndexTemplateProps } from "@/types/templates";

export default function ResourceIndexTemplate(props: ResourceIndexTemplateProps) {
  return (
    <PageShell activeNavLabel={props.activeNavLabel ?? "Resources"}>
      ...
      <section className="bg-white px-6 py-24 lg:px-8"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-start"><div><SectionIntro {...props.featuredIntro} /></div><div><ResourceFeaturedCard {...props.featuredItem} /></div></div></section>
      <section className="bg-slate-50 px-6 py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionIntro {...props.gridIntro} /><div className="mt-14 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{props.gridItems.map((item) => <ResourceCard key={item.title} {...item} />)}</div></div></section>
